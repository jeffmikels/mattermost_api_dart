//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostUpdateOutgoingWebhookRequest
void main() {
  // final instance = MattermostUpdateOutgoingWebhookRequest();

  group('test MattermostUpdateOutgoingWebhookRequest', () {
    // Outgoing webhook GUID
    // String hookId
    test('to test the property `hookId`', () async {
      // TODO
    });

    // The ID of a public channel or private group that receives the webhook payloads.
    // String channelId
    test('to test the property `channelId`', () async {
      // TODO
    });

    // The display name for this incoming webhook
    // String displayName
    test('to test the property `displayName`', () async {
      // TODO
    });

    // The description for this incoming webhook
    // String description
    test('to test the property `description`', () async {
      // TODO
    });


  });

}
