//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostPushNotification
void main() {
  // final instance = MattermostPushNotification();

  group('test MattermostPushNotification', () {
    // String ackId
    test('to test the property `ackId`', () async {
      // TODO
    });

    // String platform
    test('to test the property `platform`', () async {
      // TODO
    });

    // String serverId
    test('to test the property `serverId`', () async {
      // TODO
    });

    // String deviceId
    test('to test the property `deviceId`', () async {
      // TODO
    });

    // String postId
    test('to test the property `postId`', () async {
      // TODO
    });

    // String category
    test('to test the property `category`', () async {
      // TODO
    });

    // String sound
    test('to test the property `sound`', () async {
      // TODO
    });

    // String message
    test('to test the property `message`', () async {
      // TODO
    });

    // num badge
    test('to test the property `badge`', () async {
      // TODO
    });

    // num contAva
    test('to test the property `contAva`', () async {
      // TODO
    });

    // String teamId
    test('to test the property `teamId`', () async {
      // TODO
    });

    // String channelId
    test('to test the property `channelId`', () async {
      // TODO
    });

    // String rootId
    test('to test the property `rootId`', () async {
      // TODO
    });

    // String channelName
    test('to test the property `channelName`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // String senderId
    test('to test the property `senderId`', () async {
      // TODO
    });

    // String senderName
    test('to test the property `senderName`', () async {
      // TODO
    });

    // String overrideUsername
    test('to test the property `overrideUsername`', () async {
      // TODO
    });

    // String overrideIconUrl
    test('to test the property `overrideIconUrl`', () async {
      // TODO
    });

    // String fromWebhook
    test('to test the property `fromWebhook`', () async {
      // TODO
    });

    // String version
    test('to test the property `version`', () async {
      // TODO
    });

    // bool isIdLoaded
    test('to test the property `isIdLoaded`', () async {
      // TODO
    });


  });

}
