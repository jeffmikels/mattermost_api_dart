//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostGroupSyncableChannels
void main() {
  // final instance = MattermostGroupSyncableChannels();

  group('test MattermostGroupSyncableChannels', () {
    // String channelId
    test('to test the property `channelId`', () async {
      // TODO
    });

    // String channelDisplayName
    test('to test the property `channelDisplayName`', () async {
      // TODO
    });

    // String channelType
    test('to test the property `channelType`', () async {
      // TODO
    });

    // String teamId
    test('to test the property `teamId`', () async {
      // TODO
    });

    // String teamDisplayName
    test('to test the property `teamDisplayName`', () async {
      // TODO
    });

    // String teamType
    test('to test the property `teamType`', () async {
      // TODO
    });

    // String groupId
    test('to test the property `groupId`', () async {
      // TODO
    });

    // bool autoAdd
    test('to test the property `autoAdd`', () async {
      // TODO
    });

    // int createAt
    test('to test the property `createAt`', () async {
      // TODO
    });

    // int deleteAt
    test('to test the property `deleteAt`', () async {
      // TODO
    });

    // int updateAt
    test('to test the property `updateAt`', () async {
      // TODO
    });


  });

}
