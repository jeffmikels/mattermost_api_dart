//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostCommand
void main() {
  // final instance = MattermostCommand();

  group('test MattermostCommand', () {
    // The ID of the slash command
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // The token which is used to verify the source of the payload
    // String token
    test('to test the property `token`', () async {
      // TODO
    });

    // The time in milliseconds the command was created
    // int createAt
    test('to test the property `createAt`', () async {
      // TODO
    });

    // The time in milliseconds the command was last updated
    // int updateAt
    test('to test the property `updateAt`', () async {
      // TODO
    });

    // The time in milliseconds the command was deleted, 0 if never deleted
    // int deleteAt
    test('to test the property `deleteAt`', () async {
      // TODO
    });

    // The user id for the commands creator
    // String creatorId
    test('to test the property `creatorId`', () async {
      // TODO
    });

    // The team id for which this command is configured
    // String teamId
    test('to test the property `teamId`', () async {
      // TODO
    });

    // The string that triggers this command
    // String trigger
    test('to test the property `trigger`', () async {
      // TODO
    });

    // Is the trigger done with HTTP Get ('G') or HTTP Post ('P')
    // String method
    test('to test the property `method`', () async {
      // TODO
    });

    // What is the username for the response post
    // String username
    test('to test the property `username`', () async {
      // TODO
    });

    // The url to find the icon for this users avatar
    // String iconUrl
    test('to test the property `iconUrl`', () async {
      // TODO
    });

    // Use auto complete for this command
    // bool autoComplete
    test('to test the property `autoComplete`', () async {
      // TODO
    });

    // The description for this command shown when selecting the command
    // String autoCompleteDesc
    test('to test the property `autoCompleteDesc`', () async {
      // TODO
    });

    // The hint for this command
    // String autoCompleteHint
    test('to test the property `autoCompleteHint`', () async {
      // TODO
    });

    // Display name for the command
    // String displayName
    test('to test the property `displayName`', () async {
      // TODO
    });

    // Description for this command
    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // The URL that is triggered
    // String url
    test('to test the property `url`', () async {
      // TODO
    });


  });

}
