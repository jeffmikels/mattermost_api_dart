...
class MattermostPostsUsage {
  /// Returns a new [MattermostPostsUsage] instance.
  ...
  /// Returns a new [MattermostPostsUsage] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostPostsUsage? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostPostsUsage[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostPostsUsage[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostPostsUsage>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostPostsUsage.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostPostsUsage> mapFromJson(dynamic json) {
    final map = <String, MattermostPostsUsage>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostPostsUsage.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
