...
class MattermostAudit {
  /// Returns a new [MattermostAudit] instance.
  MattermostAudit({
    this.id,
    this.createAt,
    this.userId,
  ...
  /// Returns a new [MattermostAudit] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostAudit? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostAudit[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostAudit[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostAudit(
  ...
  static List<MattermostAudit>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostAudit.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostAudit> mapFromJson(dynamic json) {
    final map = <String, MattermostAudit>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostAudit.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
