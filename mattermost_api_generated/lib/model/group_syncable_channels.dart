...
class GroupSyncableChannels {
  /// Returns a new [GroupSyncableChannels] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is GroupSyncableChannels &&
  ...
  @override
  String toString() => 'GroupSyncableChannels[channelId=$channelId, channelDisplayName=$channelDisplayName, channelType=$channelType, teamId=$teamId, teamDisplayName=$teamDisplayName, teamType=$teamType, groupId=$groupId, autoAdd=$autoAdd, createAt=$createAt, deleteAt=$deleteAt, updateAt=$updateAt]';
  ...
  /// Returns a new [GroupSyncableChannels] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static GroupSyncableChannels? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "GroupSyncableChannels[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "GroupSyncableChannels[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return GroupSyncableChannels(
  ...
  static List<GroupSyncableChannels>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <GroupSyncableChannels>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = GroupSyncableChannels.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, GroupSyncableChannels> mapFromJson(dynamic json) {
    final map = <String, GroupSyncableChannels>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GroupSyncableChannels.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of GroupSyncableChannels-objects as value to a dart map
  static Map<String, List<GroupSyncableChannels>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<GroupSyncableChannels>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = GroupSyncableChannels.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
