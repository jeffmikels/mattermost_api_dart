...
class MattermostStatusOK {
  /// Returns a new [MattermostStatusOK] instance.
  MattermostStatusOK({
    this.status,
  });
  ...
  /// Returns a new [MattermostStatusOK] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostStatusOK? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostStatusOK[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostStatusOK[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostStatusOK(
  ...
  static List<MattermostStatusOK>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostStatusOK.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostStatusOK> mapFromJson(dynamic json) {
    final map = <String, MattermostStatusOK>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostStatusOK.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
