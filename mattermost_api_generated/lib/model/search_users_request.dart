...
class SearchUsersRequest {
  /// Returns a new [SearchUsersRequest] instance.
  SearchUsersRequest({
    required this.term,
    this.teamId,
    this.notInTeamId,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SearchUsersRequest &&
     other.term == term &&
     other.teamId == teamId &&
  ...
  @override
  String toString() => 'SearchUsersRequest[term=$term, teamId=$teamId, notInTeamId=$notInTeamId, inChannelId=$inChannelId, notInChannelId=$notInChannelId, inGroupId=$inGroupId, groupConstrained=$groupConstrained, allowInactive=$allowInactive, withoutTeam=$withoutTeam, limit=$limit]';
  ...
  /// Returns a new [SearchUsersRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SearchUsersRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SearchUsersRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SearchUsersRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SearchUsersRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SearchUsersRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SearchUsersRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SearchUsersRequest> mapFromJson(dynamic json) {
    final map = <String, SearchUsersRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchUsersRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SearchUsersRequest-objects as value to a dart map
  static Map<String, List<SearchUsersRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SearchUsersRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchUsersRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
