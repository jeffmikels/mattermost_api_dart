...
class UpdateTeamRequest {
  ...
  @override
  String toString() => 'UpdateTeamRequest[id=$id, displayName=$displayName, description=$description, companyName=$companyName, allowedDomains=$allowedDomains, inviteId=$inviteId, allowOpenInvite=$allowOpenInvite]';
  ...
  static Map<String, List<UpdateTeamRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UpdateTeamRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
