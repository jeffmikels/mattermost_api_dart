...
class MattermostChannel {
  /// Returns a new [MattermostChannel] instance.
  MattermostChannel({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  /// Returns a new [MattermostChannel] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostChannel? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostChannel[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostChannel[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostChannel>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostChannel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostChannel> mapFromJson(dynamic json) {
    final map = <String, MattermostChannel>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostChannel.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
