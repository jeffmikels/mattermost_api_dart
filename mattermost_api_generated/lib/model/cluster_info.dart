...
class ClusterInfo {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ClusterInfo &&
     other.id == id &&
  ...
  @override
  String toString() => 'ClusterInfo[id=$id, version=$version, configHash=$configHash, internodeUrl=$internodeUrl, hostname=$hostname, lastPing=$lastPing, isAlive=$isAlive]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ClusterInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ClusterInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ClusterInfo>? listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<ClusterInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ClusterInfo>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ClusterInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
