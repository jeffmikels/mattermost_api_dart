...
class MattermostAddress {
  /// Returns a new [MattermostAddress] instance.
  MattermostAddress({
    this.city,
    this.country,
    this.line1,
  ...
  /// Returns a new [MattermostAddress] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostAddress? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostAddress[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostAddress[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostAddress(
  ...
  static List<MattermostAddress>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostAddress.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostAddress> mapFromJson(dynamic json) {
    final map = <String, MattermostAddress>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostAddress.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
