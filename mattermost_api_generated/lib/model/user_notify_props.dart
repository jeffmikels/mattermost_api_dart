...
class UserNotifyProps {
  /// Returns a new [UserNotifyProps] instance.
  UserNotifyProps({
    this.email,
    this.push,
    this.desktop,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UserNotifyProps &&
     other.email == email &&
     other.push == push &&
  ...
  @override
  String toString() => 'UserNotifyProps[email=$email, push=$push, desktop=$desktop, desktopSound=$desktopSound, mentionKeys=$mentionKeys, channel=$channel, firstName=$firstName]';
  ...
  /// Returns a new [UserNotifyProps] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static UserNotifyProps? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UserNotifyProps[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UserNotifyProps[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UserNotifyProps>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <UserNotifyProps>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = UserNotifyProps.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, UserNotifyProps> mapFromJson(dynamic json) {
    final map = <String, UserNotifyProps>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserNotifyProps.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of UserNotifyProps-objects as value to a dart map
  static Map<String, List<UserNotifyProps>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UserNotifyProps>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = UserNotifyProps.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
