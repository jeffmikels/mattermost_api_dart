...
class SharedChannel {
  /// Returns a new [SharedChannel] instance.
  SharedChannel({
    this.id,
    this.teamId,
    this.home,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SharedChannel &&
     other.id == id &&
     other.teamId == teamId &&
     other.home == home &&
  ...
  @override
  String toString() => 'SharedChannel[id=$id, teamId=$teamId, home=$home, readonly=$readonly, name=$name, displayName=$displayName, purpose=$purpose, header=$header, creatorId=$creatorId, createAt=$createAt, updateAt=$updateAt, remoteId=$remoteId]';
  ...
  /// Returns a new [SharedChannel] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SharedChannel? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SharedChannel[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SharedChannel[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SharedChannel>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SharedChannel>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SharedChannel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SharedChannel> mapFromJson(dynamic json) {
    final map = <String, SharedChannel>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SharedChannel.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SharedChannel-objects as value to a dart map
  static Map<String, List<SharedChannel>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SharedChannel>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SharedChannel.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
