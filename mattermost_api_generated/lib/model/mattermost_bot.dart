...
class MattermostBot {
  /// Returns a new [MattermostBot] instance.
  MattermostBot({
    this.userId,
    this.createAt,
    this.updateAt,
  ...
  /// Returns a new [MattermostBot] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostBot? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostBot[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostBot[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostBot>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostBot.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostBot> mapFromJson(dynamic json) {
    final map = <String, MattermostBot>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostBot.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
