...
class MattermostOpenGraph {
  /// Returns a new [MattermostOpenGraph] instance.
  MattermostOpenGraph({
    this.type,
    this.url,
    this.title,
  ...
  List<MattermostOpenGraphImagesInner> images;

  List<MattermostOpenGraphVideosInner> videos;

  List<MattermostOpenGraphAudiosInner> audios;
  ...
  /// Consider adding a "default:" property in the specification file to hide this note.
  ///
  MattermostOpenGraphArticle? article;
  ...
  /// Returns a new [MattermostOpenGraph] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostOpenGraph? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostOpenGraph[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostOpenGraph[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostOpenGraph(
        ...
        article: MattermostOpenGraphArticle.fromJson(json[r'article']),
        book: MattermostOpenGraphBook.fromJson(json[r'book']),
        profile: MattermostOpenGraphArticleAuthorsInner.fromJson(json[r'profile']),
      );
    }
    return null;
  }
  ...
  static List<MattermostOpenGraph>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostOpenGraph.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostOpenGraph> mapFromJson(dynamic json) {
    final map = <String, MattermostOpenGraph>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostOpenGraph.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
