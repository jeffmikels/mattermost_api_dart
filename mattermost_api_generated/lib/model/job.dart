...
class Job {
  ...
  @override
  String toString() => 'Job[id=$id, type=$type, createAt=$createAt, startAt=$startAt, lastActivityAt=$lastActivityAt, status=$status, progress=$progress, data=$data]';
  ...
  static Map<String, List<Job>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Job.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
