...
class MattermostConfigSamlSettings {
  /// Returns a new [MattermostConfigSamlSettings] instance.
  MattermostConfigSamlSettings({
    this.enable,
    this.verify,
    this.encrypt,
    this.idpUrl,
  ...
  /// Returns a new [MattermostConfigSamlSettings] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostConfigSamlSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostConfigSamlSettings[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostConfigSamlSettings[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostConfigSamlSettings(
  ...
  static List<MattermostConfigSamlSettings>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostConfigSamlSettings.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostConfigSamlSettings> mapFromJson(dynamic json) {
    final map = <String, MattermostConfigSamlSettings>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostConfigSamlSettings.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
