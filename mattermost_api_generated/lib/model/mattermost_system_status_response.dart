...
class MattermostSystemStatusResponse {
  /// Returns a new [MattermostSystemStatusResponse] instance.
  ...
  /// Returns a new [MattermostSystemStatusResponse] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostSystemStatusResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostSystemStatusResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostSystemStatusResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostSystemStatusResponse(
  ...
  static List<MattermostSystemStatusResponse>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostSystemStatusResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostSystemStatusResponse> mapFromJson(dynamic json) {
    final map = <String, MattermostSystemStatusResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostSystemStatusResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
