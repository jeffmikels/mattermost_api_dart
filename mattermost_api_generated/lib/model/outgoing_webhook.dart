...
class OutgoingWebhook {
  /// Returns a new [OutgoingWebhook] instance.
  OutgoingWebhook({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OutgoingWebhook &&
     other.id == id &&
  ...
  @override
  String toString() => 'OutgoingWebhook[id=$id, createAt=$createAt, updateAt=$updateAt, deleteAt=$deleteAt, creatorId=$creatorId, teamId=$teamId, channelId=$channelId, description=$description, displayName=$displayName, triggerWords=$triggerWords, triggerWhen=$triggerWhen, callbackUrls=$callbackUrls, contentType=$contentType]';
  ...
  /// Returns a new [OutgoingWebhook] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OutgoingWebhook? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OutgoingWebhook[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OutgoingWebhook[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OutgoingWebhook>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OutgoingWebhook>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OutgoingWebhook.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OutgoingWebhook> mapFromJson(dynamic json) {
    final map = <String, OutgoingWebhook>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OutgoingWebhook.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OutgoingWebhook-objects as value to a dart map
  static Map<String, List<OutgoingWebhook>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OutgoingWebhook>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OutgoingWebhook.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
