...
class MattermostConfigGitLabSettings {
  /// Returns a new [MattermostConfigGitLabSettings] instance.
  MattermostConfigGitLabSettings({
    this.enable,
    this.secret,
    this.id,
    this.scope,
  ...
  /// Returns a new [MattermostConfigGitLabSettings] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostConfigGitLabSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostConfigGitLabSettings[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostConfigGitLabSettings[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostConfigGitLabSettings(
  ...
  static List<MattermostConfigGitLabSettings>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostConfigGitLabSettings.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostConfigGitLabSettings> mapFromJson(dynamic json) {
    final map = <String, MattermostConfigGitLabSettings>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostConfigGitLabSettings.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
