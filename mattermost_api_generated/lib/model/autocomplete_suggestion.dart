...
class AutocompleteSuggestion {
  ...
  @override
  String toString() => 'AutocompleteSuggestion[complete=$complete, suggestion=$suggestion, hint=$hint, description=$description, iconData=$iconData]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AutocompleteSuggestion[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AutocompleteSuggestion[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static Map<String, List<AutocompleteSuggestion>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AutocompleteSuggestion.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
