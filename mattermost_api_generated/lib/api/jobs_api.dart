...
class JobsApi {
  ...
  /// * [CreateJobRequest] createJobRequest (required):
  ///   Job object to be created
  Future<Job?> createJob(CreateJobRequest createJobRequest,) async {
  ...
  /// * [int] perPage:
  ///   The number of jobs per page.
  Future<List<Job>?> getJobs({ int? page, int? perPage, }) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<Job>') as List)
        .cast<Job>()
        .toList();
  ...
  /// * [int] perPage:
  ///   The number of jobs per page.
  Future<List<Job>?> getJobsByType(String type, { int? page, int? perPage, }) async {
