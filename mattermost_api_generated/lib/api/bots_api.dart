...
class BotsApi {
  ...
  Future<Bot?> assignBot(String botUserId, String userId,) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Bot',) as Bot;
  ...
  /// Parameters:
  ///
  /// * [CreateBotRequest] createBotRequest (required):
  ///   Bot to be created
  Future<Response> createBotWithHttpInfo(CreateBotRequest createBotRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/bots';

    // ignore: prefer_final_locals
    Object? postBody = createBotRequest;
  ...
  Future<Bot?> createBot(CreateBotRequest createBotRequest,) async {
    final response = await createBotWithHttpInfo(createBotRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<List<Bot>?> getBots({ int? page, int? perPage, bool? includeDeleted, bool? onlyOrphaned, }) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<Bot>') as List)
        .cast<Bot>()
        .toList();
  ...
  /// * [CreateBotRequest] createBotRequest (required):
  ///   Bot to be created
  Future<Response> patchBotWithHttpInfo(String botUserId, CreateBotRequest createBotRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/bots/{bot_user_id}'
      .replaceAll('{bot_user_id}', botUserId);
  ...
  Future<Bot?> patchBot(String botUserId, CreateBotRequest createBotRequest,) async {
    final response = await patchBotWithHttpInfo(botUserId, createBotRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
