...
class PluginsApi {
  ...
  Future<List<MarketplacePlugin>?> getMarketplacePlugins({ int? page, int? perPage, String? filter, String? serverVersion, bool? localOnly, }) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<MarketplacePlugin>') as List)
  ...
  Future<System?> getMarketplaceVisitedByAdmin() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'System',) as System;
  ...
  Future<List<PluginStatus>?> getPluginStatuses() async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      final responseBody = await _decodeBodyBytes(response);
      return (await apiClient.deserializeAsync(responseBody, 'List<PluginStatus>') as List)
        .cast<PluginStatus>()
        .toList();
  ...
  Future<GetPlugins200Response?> getPlugins() async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'GetPlugins200Response',) as GetPlugins200Response;
  ...
  Future<List<PluginManifestWebapp>?> getWebappPlugins() async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      final responseBody = await _decodeBodyBytes(response);
      return (await apiClient.deserializeAsync(responseBody, 'List<PluginManifestWebapp>') as List)
        .cast<PluginManifestWebapp>()
        .toList();
  ...
  /// Parameters:
  ///
  /// * [InstallMarketplacePluginRequest] installMarketplacePluginRequest (required):
  ///   The metadata identifying the plugin to install.
  Future<Response> installMarketplacePluginWithHttpInfo(InstallMarketplacePluginRequest installMarketplacePluginRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/plugins/marketplace';

    // ignore: prefer_final_locals
    Object? postBody = installMarketplacePluginRequest;
  ...
  Future<PluginManifest?> installMarketplacePlugin(InstallMarketplacePluginRequest installMarketplacePluginRequest,) async {
    final response = await installMarketplacePluginWithHttpInfo(installMarketplacePluginRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'PluginManifest',) as PluginManifest;
