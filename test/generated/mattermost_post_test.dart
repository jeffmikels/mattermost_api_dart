//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostPost
void main() {
  // final instance = MattermostPost();

  group('test MattermostPost', () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // The time in milliseconds a post was created
    // int createAt
    test('to test the property `createAt`', () async {
      // TODO
    });

    // The time in milliseconds a post was last updated
    // int updateAt
    test('to test the property `updateAt`', () async {
      // TODO
    });

    // The time in milliseconds a post was deleted
    // int deleteAt
    test('to test the property `deleteAt`', () async {
      // TODO
    });

    // int editAt
    test('to test the property `editAt`', () async {
      // TODO
    });

    // String userId
    test('to test the property `userId`', () async {
      // TODO
    });

    // String channelId
    test('to test the property `channelId`', () async {
      // TODO
    });

    // String rootId
    test('to test the property `rootId`', () async {
      // TODO
    });

    // String originalId
    test('to test the property `originalId`', () async {
      // TODO
    });

    // String message
    test('to test the property `message`', () async {
      // TODO
    });

    // String type
    test('to test the property `type`', () async {
      // TODO
    });

    // Map props
    test('to test the property `props`', () async {
      // TODO
    });

    // String hashtag
    test('to test the property `hashtag`', () async {
      // TODO
    });

    // List<String> fileIds (default value: const [])
    test('to test the property `fileIds`', () async {
      // TODO
    });

    // String pendingPostId
    test('to test the property `pendingPostId`', () async {
      // TODO
    });

    // MattermostPostMetadata metadata
    test('to test the property `metadata`', () async {
      // TODO
    });


  });

}
