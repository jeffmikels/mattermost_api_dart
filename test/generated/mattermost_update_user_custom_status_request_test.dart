//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostUpdateUserCustomStatusRequest
void main() {
  // final instance = MattermostUpdateUserCustomStatusRequest();

  group('test MattermostUpdateUserCustomStatusRequest', () {
    // Any emoji
    // String emoji
    test('to test the property `emoji`', () async {
      // TODO
    });

    // Any custom status text
    // String text
    test('to test the property `text`', () async {
      // TODO
    });

    // Duration of custom status, can be `thirty_minutes`, `one_hour`, `four_hours`, `today`, `this_week` or `date_and_time`
    // String duration
    test('to test the property `duration`', () async {
      // TODO
    });

    // The time at which custom status should be expired. It should be in ISO format.
    // String expiresAt
    test('to test the property `expiresAt`', () async {
      // TODO
    });


  });

}
