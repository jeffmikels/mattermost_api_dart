//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:mattermost_api/api.dart';
import 'package:test/test.dart';

// tests for MattermostPostList
void main() {
  // final instance = MattermostPostList();

  group('test MattermostPostList', () {
    // List<String> order (default value: const [])
    test('to test the property `order`', () async {
      // TODO
    });

    // Map<String, MattermostPost> posts (default value: const {})
    test('to test the property `posts`', () async {
      // TODO
    });

    // The ID of next post. Not omitted when empty or not relevant.
    // String nextPostId
    test('to test the property `nextPostId`', () async {
      // TODO
    });

    // The ID of previous post. Not omitted when empty or not relevant.
    // String prevPostId
    test('to test the property `prevPostId`', () async {
      // TODO
    });

    // Whether there are more items after this page.
    // bool hasNext
    test('to test the property `hasNext`', () async {
      // TODO
    });


  });

}
