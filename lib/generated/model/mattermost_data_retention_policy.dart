...
class MattermostDataRetentionPolicy {
  /// Returns a new [MattermostDataRetentionPolicy] instance.
  MattermostDataRetentionPolicy({
    this.displayName,
    this.postDuration,
    this.id,
  });
  ...
  /// Returns a new [MattermostDataRetentionPolicy] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostDataRetentionPolicy? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostDataRetentionPolicy[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostDataRetentionPolicy[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostDataRetentionPolicy(
  ...
  static List<MattermostDataRetentionPolicy>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostDataRetentionPolicy.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostDataRetentionPolicy> mapFromJson(dynamic json) {
    final map = <String, MattermostDataRetentionPolicy>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostDataRetentionPolicy.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
