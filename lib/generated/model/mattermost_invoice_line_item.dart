...
class MattermostInvoiceLineItem {
  /// Returns a new [MattermostInvoiceLineItem] instance.
  MattermostInvoiceLineItem({
    this.priceId,
    this.total,
    this.quantity,
    this.pricePerUnit,
  ...
  /// Returns a new [MattermostInvoiceLineItem] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostInvoiceLineItem? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostInvoiceLineItem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostInvoiceLineItem[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostInvoiceLineItem(
  ...
  static List<MattermostInvoiceLineItem>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostInvoiceLineItem.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostInvoiceLineItem> mapFromJson(dynamic json) {
    final map = <String, MattermostInvoiceLineItem>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostInvoiceLineItem.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
