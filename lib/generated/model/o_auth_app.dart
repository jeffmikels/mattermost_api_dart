...
class OAuthApp {
  /// Returns a new [OAuthApp] instance.
  OAuthApp({
    this.id,
    this.clientSecret,
    this.name,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is OAuthApp &&
     other.id == id &&
  ...
  @override
  String toString() => 'OAuthApp[id=$id, clientSecret=$clientSecret, name=$name, description=$description, iconUrl=$iconUrl, callbackUrls=$callbackUrls, homepage=$homepage, isTrusted=$isTrusted, createAt=$createAt, updateAt=$updateAt]';
  ...
  /// Returns a new [OAuthApp] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static OAuthApp? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "OAuthApp[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "OAuthApp[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<OAuthApp>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <OAuthApp>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = OAuthApp.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, OAuthApp> mapFromJson(dynamic json) {
    final map = <String, OAuthApp>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OAuthApp.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of OAuthApp-objects as value to a dart map
  static Map<String, List<OAuthApp>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<OAuthApp>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = OAuthApp.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
