...
class ChannelWithTeamData {
  /// Returns a new [ChannelWithTeamData] instance.
  ChannelWithTeamData({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ChannelWithTeamData &&
     other.id == id &&
  ...
  @override
  String toString() => 'ChannelWithTeamData[id=$id, createAt=$createAt, updateAt=$updateAt, deleteAt=$deleteAt, teamId=$teamId, type=$type, displayName=$displayName, name=$name, header=$header, purpose=$purpose, lastPostAt=$lastPostAt, totalMsgCount=$totalMsgCount, extraUpdateAt=$extraUpdateAt, creatorId=$creatorId, teamDisplayName=$teamDisplayName, teamName=$teamName, teamUpdateAt=$teamUpdateAt, policyId=$policyId]';
  ...
  /// Returns a new [ChannelWithTeamData] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ChannelWithTeamData? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ChannelWithTeamData[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ChannelWithTeamData[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return ChannelWithTeamData(
  ...
  static List<ChannelWithTeamData>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ChannelWithTeamData>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ChannelWithTeamData.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ChannelWithTeamData> mapFromJson(dynamic json) {
    final map = <String, ChannelWithTeamData>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ChannelWithTeamData.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ChannelWithTeamData-objects as value to a dart map
  static Map<String, List<ChannelWithTeamData>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ChannelWithTeamData>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ChannelWithTeamData.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
