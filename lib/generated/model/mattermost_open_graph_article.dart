...
class MattermostOpenGraphArticle {
  /// Returns a new [MattermostOpenGraphArticle] instance.
  MattermostOpenGraphArticle({
    this.publishedTime,
    this.modifiedTime,
    this.expirationTime,
    this.section,
  ...
  /// Returns a new [MattermostOpenGraphArticle] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostOpenGraphArticle? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostOpenGraphArticle[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostOpenGraphArticle[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostOpenGraphArticle(
  ...
  static List<MattermostOpenGraphArticle>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostOpenGraphArticle.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostOpenGraphArticle> mapFromJson(dynamic json) {
    final map = <String, MattermostOpenGraphArticle>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostOpenGraphArticle.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
