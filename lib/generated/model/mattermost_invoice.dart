...
class MattermostInvoice {
  /// Returns a new [MattermostInvoice] instance.
  MattermostInvoice({
    this.id,
    this.number,
    this.createAt,
  ...
  /// Returns a new [MattermostInvoice] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostInvoice? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostInvoice[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostInvoice[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostInvoice(
        ...
        item: MattermostInvoiceLineItem.listFromJson(json[r'item']) ?? const [],
      );
    }
    return null;
  }
  ...
  static List<MattermostInvoice>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostInvoice.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostInvoice> mapFromJson(dynamic json) {
    final map = <String, MattermostInvoice>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostInvoice.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
