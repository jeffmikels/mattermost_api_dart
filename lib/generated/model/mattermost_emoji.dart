...
class MattermostEmoji {
  /// Returns a new [MattermostEmoji] instance.
  MattermostEmoji({
    this.id,
    this.creatorId,
    this.name,
  ...
  /// Returns a new [MattermostEmoji] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostEmoji? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostEmoji[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostEmoji[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostEmoji(
  ...
  static List<MattermostEmoji>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostEmoji.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostEmoji> mapFromJson(dynamic json) {
    final map = <String, MattermostEmoji>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostEmoji.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
