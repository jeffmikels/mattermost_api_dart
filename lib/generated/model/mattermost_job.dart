...
class MattermostJob {
  /// Returns a new [MattermostJob] instance.
  MattermostJob({
    this.id,
    this.type,
    this.createAt,
  ...
  /// Returns a new [MattermostJob] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostJob? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostJob[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostJob[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostJob(
  ...
  static List<MattermostJob>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostJob.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostJob> mapFromJson(dynamic json) {
    final map = <String, MattermostJob>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostJob.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
