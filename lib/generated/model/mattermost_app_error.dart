...
class MattermostAppError {
  /// Returns a new [MattermostAppError] instance.
  MattermostAppError({
    this.statusCode,
    this.id,
    this.message,
  ...
  /// Returns a new [MattermostAppError] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostAppError? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostAppError[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostAppError[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostAppError(
  ...
  static List<MattermostAppError>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostAppError.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostAppError> mapFromJson(dynamic json) {
    final map = <String, MattermostAppError>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostAppError.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
