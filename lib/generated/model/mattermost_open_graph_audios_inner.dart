...
class MattermostOpenGraphAudiosInner {
  /// Returns a new [MattermostOpenGraphAudiosInner] instance.
  MattermostOpenGraphAudiosInner({
    this.url,
    this.secureUrl,
    this.type,
  });
  ...
  /// Returns a new [MattermostOpenGraphAudiosInner] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostOpenGraphAudiosInner? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostOpenGraphAudiosInner[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostOpenGraphAudiosInner[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostOpenGraphAudiosInner(
  ...
  static List<MattermostOpenGraphAudiosInner>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostOpenGraphAudiosInner.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostOpenGraphAudiosInner> mapFromJson(dynamic json) {
    final map = <String, MattermostOpenGraphAudiosInner>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostOpenGraphAudiosInner.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
