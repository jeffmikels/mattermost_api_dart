...
class FileInfo {
  ...
  @override
  String toString() => 'FileInfo[id=$id, userId=$userId, postId=$postId, createAt=$createAt, updateAt=$updateAt, deleteAt=$deleteAt, name=$name, extension_=$extension_, size=$size, mimeType=$mimeType, width=$width, height=$height, hasPreviewImage=$hasPreviewImage]';
  ...
  static Map<String, List<FileInfo>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = FileInfo.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
