...
class MattermostScheme {
  /// Returns a new [MattermostScheme] instance.
  MattermostScheme({
    this.id,
    this.name,
    this.description,
  ...
  /// Returns a new [MattermostScheme] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostScheme? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostScheme[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostScheme[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostScheme(
  ...
  static List<MattermostScheme>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostScheme.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostScheme> mapFromJson(dynamic json) {
    final map = <String, MattermostScheme>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostScheme.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
