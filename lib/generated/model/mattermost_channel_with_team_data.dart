...
class MattermostChannelWithTeamData {
  /// Returns a new [MattermostChannelWithTeamData] instance.
  MattermostChannelWithTeamData({
    this.id,
    this.createAt,
    this.updateAt,
    this.deleteAt,
  ...
  /// Returns a new [MattermostChannelWithTeamData] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostChannelWithTeamData? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostChannelWithTeamData[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostChannelWithTeamData[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostChannelWithTeamData(
  ...
  static List<MattermostChannelWithTeamData>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostChannelWithTeamData.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostChannelWithTeamData> mapFromJson(dynamic json) {
    final map = <String, MattermostChannelWithTeamData>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostChannelWithTeamData.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
