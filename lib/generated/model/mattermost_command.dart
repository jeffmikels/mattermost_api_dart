...
class MattermostCommand {
  /// Returns a new [MattermostCommand] instance.
  MattermostCommand({
    this.id,
    this.token,
    this.createAt,
  ...
  /// Returns a new [MattermostCommand] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostCommand? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostCommand[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostCommand[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostCommand>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostCommand.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostCommand> mapFromJson(dynamic json) {
    final map = <String, MattermostCommand>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostCommand.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
