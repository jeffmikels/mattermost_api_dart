...
class MattermostTopChannel {
  /// Returns a new [MattermostTopChannel] instance.
  MattermostTopChannel({
    this.id,
    this.type,
    this.displayName,
    this.name,
  ...
  /// Returns a new [MattermostTopChannel] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostTopChannel? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostTopChannel[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostTopChannel[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostTopChannel(
  ...
  static List<MattermostTopChannel>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostTopChannel.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostTopChannel> mapFromJson(dynamic json) {
    final map = <String, MattermostTopChannel>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostTopChannel.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
