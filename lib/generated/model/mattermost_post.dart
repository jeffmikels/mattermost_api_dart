...
class MattermostPost {
  /// Returns a new [MattermostPost] instance.
  MattermostPost({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  /// Returns a new [MattermostPost] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostPost? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostPost[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostPost[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostPost>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostPost.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostPost> mapFromJson(dynamic json) {
    final map = <String, MattermostPost>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostPost.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
