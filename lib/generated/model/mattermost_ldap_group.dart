...
class MattermostLDAPGroup {
  /// Returns a new [MattermostLDAPGroup] instance.
  MattermostLDAPGroup({
    this.hasSyncables,
  ...
  /// Returns a new [MattermostLDAPGroup] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostLDAPGroup? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostLDAPGroup[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostLDAPGroup[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostLDAPGroup(
  ...
  static List<MattermostLDAPGroup>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostLDAPGroup.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostLDAPGroup> mapFromJson(dynamic json) {
    final map = <String, MattermostLDAPGroup>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostLDAPGroup.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
