...
class MattermostUser {
  /// Returns a new [MattermostUser] instance.
  MattermostUser({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  /// Returns a new [MattermostUser] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostUser? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostUser[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostUser[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostUser>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostUser.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostUser> mapFromJson(dynamic json) {
    final map = <String, MattermostUser>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostUser.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
