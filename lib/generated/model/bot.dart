...
class Bot {
  ...
  @override
  String toString() => 'Bot[userId=$userId, createAt=$createAt, updateAt=$updateAt, deleteAt=$deleteAt, username=$username, displayName=$displayName, description=$description, ownerId=$ownerId]';
  ...
  static Bot? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Bot[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Bot[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Bot>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Bot>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Bot.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Bot> mapFromJson(dynamic json) {
    final map = <String, Bot>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Bot.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  static Map<String, List<Bot>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Bot>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Bot.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
