...
class MattermostClusterInfo {
  /// Returns a new [MattermostClusterInfo] instance.
  MattermostClusterInfo({
    this.id,
    this.version,
    this.configHash,
    this.internodeUrl,
  ...
  /// Returns a new [MattermostClusterInfo] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostClusterInfo? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostClusterInfo[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostClusterInfo[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostClusterInfo(
  ...
  static List<MattermostClusterInfo>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostClusterInfo.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostClusterInfo> mapFromJson(dynamic json) {
    final map = <String, MattermostClusterInfo>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostClusterInfo.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
