...
class Scheme {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Scheme &&
     other.id == id &&
     other.name == name &&
  ...
  @override
  String toString() => 'Scheme[id=$id, name=$name, description=$description, createAt=$createAt, updateAt=$updateAt, deleteAt=$deleteAt, scope=$scope, defaultTeamAdminRole=$defaultTeamAdminRole, defaultTeamUserRole=$defaultTeamUserRole, defaultChannelAdminRole=$defaultChannelAdminRole, defaultChannelUserRole=$defaultChannelUserRole]';
  ...
  /// Returns a new [Scheme] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static Scheme? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Scheme[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Scheme[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<Scheme>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <Scheme>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = Scheme.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, Scheme> mapFromJson(dynamic json) {
    final map = <String, Scheme>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Scheme.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of Scheme-objects as value to a dart map
  static Map<String, List<Scheme>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Scheme>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Scheme.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
