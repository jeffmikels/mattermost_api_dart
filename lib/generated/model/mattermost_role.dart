...
class MattermostRole {
  /// Returns a new [MattermostRole] instance.
  MattermostRole({
    this.id,
    this.name,
    this.displayName,
  ...
  /// Returns a new [MattermostRole] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostRole? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostRole[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostRole[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostRole(
  ...
  static List<MattermostRole>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostRole.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostRole> mapFromJson(dynamic json) {
    final map = <String, MattermostRole>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostRole.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
