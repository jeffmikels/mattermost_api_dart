...
class MattermostUserNotifyProps {
  /// Returns a new [MattermostUserNotifyProps] instance.
  MattermostUserNotifyProps({
    this.email,
    this.push,
    this.desktop,
    this.desktopSound,
  ...
  /// Returns a new [MattermostUserNotifyProps] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostUserNotifyProps? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostUserNotifyProps[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostUserNotifyProps[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostUserNotifyProps(
  ...
  static List<MattermostUserNotifyProps>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostUserNotifyProps.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostUserNotifyProps> mapFromJson(dynamic json) {
    final map = <String, MattermostUserNotifyProps>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostUserNotifyProps.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
