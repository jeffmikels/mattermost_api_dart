...
class MattermostSubscription {
  /// Returns a new [MattermostSubscription] instance.
  ...
  /// Returns a new [MattermostSubscription] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostSubscription? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostSubscription[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostSubscription[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostSubscription(
  ...
  static List<MattermostSubscription>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostSubscription.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostSubscription> mapFromJson(dynamic json) {
    final map = <String, MattermostSubscription>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostSubscription.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
