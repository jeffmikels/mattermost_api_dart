...
class MattermostConfigPasswordSettings {
  /// Returns a new [MattermostConfigPasswordSettings] instance.
  MattermostConfigPasswordSettings({
    this.minimumLength,
    this.lowercase,
    this.number,
    this.uppercase,
  ...
  /// Returns a new [MattermostConfigPasswordSettings] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostConfigPasswordSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostConfigPasswordSettings[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostConfigPasswordSettings[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostConfigPasswordSettings(
  ...
  static List<MattermostConfigPasswordSettings>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostConfigPasswordSettings.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostConfigPasswordSettings> mapFromJson(dynamic json) {
    final map = <String, MattermostConfigPasswordSettings>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostConfigPasswordSettings.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
