...
class MattermostOrphanedRecord {
  /// Returns a new [MattermostOrphanedRecord] instance.
  MattermostOrphanedRecord({
    this.parentId,
    this.childId,
  });
  ...
  /// Returns a new [MattermostOrphanedRecord] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostOrphanedRecord? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostOrphanedRecord[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostOrphanedRecord[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostOrphanedRecord(
  ...
  static List<MattermostOrphanedRecord>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostOrphanedRecord.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostOrphanedRecord> mapFromJson(dynamic json) {
    final map = <String, MattermostOrphanedRecord>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostOrphanedRecord.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
