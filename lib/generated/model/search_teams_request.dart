...
class SearchTeamsRequest {
  /// Returns a new [SearchTeamsRequest] instance.
  SearchTeamsRequest({
    this.term,
    this.page,
    this.perPage,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SearchTeamsRequest &&
     other.term == term &&
     other.page == page &&
  ...
  @override
  String toString() => 'SearchTeamsRequest[term=$term, page=$page, perPage=$perPage, allowOpenInvite=$allowOpenInvite, groupConstrained=$groupConstrained, excludePolicyConstrained=$excludePolicyConstrained]';
  ...
  /// Returns a new [SearchTeamsRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SearchTeamsRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SearchTeamsRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SearchTeamsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return SearchTeamsRequest(
  ...
  static List<SearchTeamsRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SearchTeamsRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SearchTeamsRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SearchTeamsRequest> mapFromJson(dynamic json) {
    final map = <String, SearchTeamsRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchTeamsRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SearchTeamsRequest-objects as value to a dart map
  static Map<String, List<SearchTeamsRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SearchTeamsRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchTeamsRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
