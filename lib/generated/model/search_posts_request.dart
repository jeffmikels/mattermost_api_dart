...
class SearchPostsRequest {
  /// Returns a new [SearchPostsRequest] instance.
  SearchPostsRequest({
    required this.terms,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is SearchPostsRequest &&
     other.terms == terms &&
  ...
  @override
  String toString() => 'SearchPostsRequest[terms=$terms, isOrSearch=$isOrSearch, timeZoneOffset=$timeZoneOffset, includeDeletedChannels=$includeDeletedChannels, page=$page, perPage=$perPage]';
  ...
  /// Returns a new [SearchPostsRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static SearchPostsRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "SearchPostsRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "SearchPostsRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<SearchPostsRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    final result = <SearchPostsRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = SearchPostsRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, SearchPostsRequest> mapFromJson(dynamic json) {
    final map = <String, SearchPostsRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchPostsRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of SearchPostsRequest-objects as value to a dart map
  static Map<String, List<SearchPostsRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<SearchPostsRequest>>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = SearchPostsRequest.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
