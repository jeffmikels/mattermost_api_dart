...
class Audit {
  ...
  @override
  String toString() => 'Audit[id=$id, createAt=$createAt, userId=$userId, action=$action, extraInfo=$extraInfo, ipAddress=$ipAddress, sessionId=$sessionId]';
  ...
  static Map<String, List<Audit>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = Audit.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
