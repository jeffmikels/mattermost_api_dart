...
class MattermostSearchUsersRequest {
  /// Returns a new [MattermostSearchUsersRequest] instance.
  ...
  /// Returns a new [MattermostSearchUsersRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostSearchUsersRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostSearchUsersRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostSearchUsersRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostSearchUsersRequest(
  ...
  static List<MattermostSearchUsersRequest>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostSearchUsersRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostSearchUsersRequest> mapFromJson(dynamic json) {
    final map = <String, MattermostSearchUsersRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostSearchUsersRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
