...
class MattermostTeam {
  /// Returns a new [MattermostTeam] instance.
  MattermostTeam({
    this.id,
    this.createAt,
    this.updateAt,
  ...
  /// Returns a new [MattermostTeam] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostTeam? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostTeam[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostTeam[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<MattermostTeam>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostTeam.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostTeam> mapFromJson(dynamic json) {
    final map = <String, MattermostTeam>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostTeam.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
