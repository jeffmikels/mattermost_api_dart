...
class MattermostPaymentMethod {
  /// Returns a new [MattermostPaymentMethod] instance.
  MattermostPaymentMethod({
    this.type,
    this.lastFour,
    this.expMonth,
    this.expYear,
  ...
  /// Returns a new [MattermostPaymentMethod] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostPaymentMethod? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostPaymentMethod[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostPaymentMethod[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostPaymentMethod(
  ...
  static List<MattermostPaymentMethod>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostPaymentMethod.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostPaymentMethod> mapFromJson(dynamic json) {
    final map = <String, MattermostPaymentMethod>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostPaymentMethod.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
