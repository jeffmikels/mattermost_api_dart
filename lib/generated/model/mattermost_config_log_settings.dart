...
class MattermostConfigLogSettings {
  /// Returns a new [MattermostConfigLogSettings] instance.
  MattermostConfigLogSettings({
    this.enableConsole,
    this.consoleLevel,
    this.enableFile,
    this.fileLevel,
  ...
  /// Returns a new [MattermostConfigLogSettings] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostConfigLogSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostConfigLogSettings[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostConfigLogSettings[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostConfigLogSettings(
  ...
  static List<MattermostConfigLogSettings>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostConfigLogSettings.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostConfigLogSettings> mapFromJson(dynamic json) {
    final map = <String, MattermostConfigLogSettings>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostConfigLogSettings.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
