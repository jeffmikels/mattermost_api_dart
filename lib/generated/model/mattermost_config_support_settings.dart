...
class MattermostConfigSupportSettings {
  /// Returns a new [MattermostConfigSupportSettings] instance.
  MattermostConfigSupportSettings({
  ...
  /// Returns a new [MattermostConfigSupportSettings] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostConfigSupportSettings? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostConfigSupportSettings[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostConfigSupportSettings[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostConfigSupportSettings(
  ...
  static List<MattermostConfigSupportSettings>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostConfigSupportSettings.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostConfigSupportSettings> mapFromJson(dynamic json) {
    final map = <String, MattermostConfigSupportSettings>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostConfigSupportSettings.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
