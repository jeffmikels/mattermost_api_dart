...
class PaymentMethod {
  ...
  @override
  String toString() => 'PaymentMethod[type=$type, lastFour=$lastFour, expMonth=$expMonth, expYear=$expYear, cardBrand=$cardBrand, name=$name]';
  ...
  static Map<String, List<PaymentMethod>> mapListFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PaymentMethod.listFromJson(entry.value, growable: growable,);
        if (value != null) {
          map[entry.key] = value;
        }
      }
