...
class MattermostAutocompleteSuggestion {
  /// Returns a new [MattermostAutocompleteSuggestion] instance.
  MattermostAutocompleteSuggestion({
    this.complete,
    this.suggestion,
    this.hint,
    this.description,
  ...
  /// Returns a new [MattermostAutocompleteSuggestion] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostAutocompleteSuggestion? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostAutocompleteSuggestion[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostAutocompleteSuggestion[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostAutocompleteSuggestion(
  ...
  static List<MattermostAutocompleteSuggestion>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostAutocompleteSuggestion.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostAutocompleteSuggestion> mapFromJson(dynamic json) {
    final map = <String, MattermostAutocompleteSuggestion>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostAutocompleteSuggestion.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
