...
class MattermostPluginManifestBackend {
  /// Returns a new [MattermostPluginManifestBackend] instance.
  MattermostPluginManifestBackend({
    this.executable,
  });
  ...
  /// Returns a new [MattermostPluginManifestBackend] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static MattermostPluginManifestBackend? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "MattermostPluginManifestBackend[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "MattermostPluginManifestBackend[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return MattermostPluginManifestBackend(
  ...
  static List<MattermostPluginManifestBackend>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = MattermostPluginManifestBackend.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, MattermostPluginManifestBackend> mapFromJson(dynamic json) {
    final map = <String, MattermostPluginManifestBackend>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = MattermostPluginManifestBackend.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
