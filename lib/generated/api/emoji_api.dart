...
class EmojiApi {
  EmojiApi([ApiClient? apiClient]) : apiClient = apiClient ?? defaultApiClient;
  ...
  Future<Emoji?> autocompleteEmoji(String name,) async {
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'Emoji',) as Emoji;
  ...
  /// Parameters:
  ///
  /// * [SearchEmojiRequest] searchEmojiRequest (required):
  ///   Search criteria
  Future<Response> searchEmojiWithHttpInfo(SearchEmojiRequest searchEmojiRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/emoji/search';

    // ignore: prefer_final_locals
    Object? postBody = searchEmojiRequest;
  ...
  /// * [SearchEmojiRequest] searchEmojiRequest (required):
  ///   Search criteria
  Future<List<Emoji>?> searchEmoji(SearchEmojiRequest searchEmojiRequest,) async {
    final response = await searchEmojiWithHttpInfo(searchEmojiRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      ...
      return (await apiClient.deserializeAsync(responseBody, 'List<Emoji>') as List)
        .cast<Emoji>()
        .toList();
