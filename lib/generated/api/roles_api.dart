...
class RolesApi {
  ...
  ///   Role GUID
  ///
  /// * [PatchRoleRequest] patchRoleRequest (required):
  ///   Role object to be updated
  Future<Response> patchRoleWithHttpInfo(String roleId, PatchRoleRequest patchRoleRequest,) async {
    // ignore: prefer_const_declarations
    final path = r'/roles/{role_id}/patch'
      .replaceAll('{role_id}', roleId);

    // ignore: prefer_final_locals
    Object? postBody = patchRoleRequest;
  ...
  Future<Role?> patchRole(String roleId, PatchRoleRequest patchRoleRequest,) async {
    final response = await patchRoleWithHttpInfo(roleId, patchRoleRequest,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
